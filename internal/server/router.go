...
func (s *Server) Handler() http.Handler {
	...
	mux.HandleFunc("POST /api/admin/sell/{id}/manual-pix-paid", s.handleAdminSellManualPixPaid)
	...
	mux.HandleFunc("GET /.well-known/agent-policy.json", s.handleAgentPolicyDiscoveryWellKnown)
	mux.HandleFunc("GET /.well-known/capability-graph.json", s.handleAgentCapabilityGraphWellKnown)
	mux.HandleFunc("GET /.well-known/capability-compositions.json", s.handleAgentCapabilityCompositionsWellKnown)
	mux.HandleFunc("GET /.well-known/capability-graph-registry.json", s.handleCapabilityGraphRegistryWellKnown)
	...
	mux.HandleFunc("GET /agent/v1/policy-discovery", s.handleAgentPolicyDiscovery)
	mux.HandleFunc("GET /agent/v1/capability-graph", s.handleAgentCapabilityGraph)
	mux.HandleFunc("GET /agent/v1/capability-compositions", s.handleAgentCapabilityCompositions)
	mux.HandleFunc("POST /agent/v1/plans", s.handleAgentPlanCreate)
	...
	mux.HandleFunc("GET /agent/v1/graph-registry", s.handleAgentGraphRegistry)
	...
	return securityHeaders(s.cfg, cors(s.cfg, withRequestID(s.withServerTiming(s.withDeveloperRequestLog(s.withPublicSurfaceGuards(s.withSmartRateLimit(mux)))))))
}
