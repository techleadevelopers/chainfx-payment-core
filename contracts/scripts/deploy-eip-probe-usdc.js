...
function compileContract() {
  const sourcePath = path.join(__dirname, "..", "src", "mocks", "MockUSDC3009.sol");
  const source = fs.readFileSync(sourcePath, "utf8");
  const input = {
    language: "Solidity",
    sources: {
      "MockUSDC3009.sol": { content: source }
    },
    settings: {
  ...
  const contract = output.contracts["MockUSDC3009.sol"].MockUSDC3009;
  return {
    abi: contract.abi,
...
async function main() {
  ...
  const name = process.env.EIP_PROBE_USDC_NAME || "USD Coin";
  const symbol = process.env.EIP_PROBE_USDC_SYMBOL || "USDC";
  const version = process.env.EIP_PROBE_USDC_VERSION || "2";
  ...
  console.log("MockUSDC3009:", tokenAddress);
  ...
  console.log(`EIP_PROBE_USDC_CONTRACT=${tokenAddress}`);
  console.log(`EIP_PROBE_USDC_NAME=${name}`);
  console.log(`EIP_PROBE_USDC_VERSION=${version}`);
